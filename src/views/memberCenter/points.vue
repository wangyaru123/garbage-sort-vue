  ...
  <div class="p-10">
    ...
    <el-table :data="currentTableData" border stripe class="mt-10">
      ...
      <el-table-column label="消费积分" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.consumePoints}}</span>
        </template>
      </el-table-column>
      <el-table-column label="当前积分" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.currentPoints}}</span>
        </template>
      </el-table-column>
...
export default {
  data() {
    return {
      ...
      // table所有数据
      tableData: [
        { userId: 1, username: 'wangyi', name: '王一', mobile: '15076541233', totalPoints: 100, consumePoints: 50, currentPoints: 50, projectId: 1 },
        { userId: 2, username: 'zhangshan', name: '张山', mobile: '15076541231', totalPoints: 90, consumePoints: 40, currentPoints: 50, projectId: 2 }
      ],
      // 当前tableData数据
      currentTableData: [],
      // 项目列表
